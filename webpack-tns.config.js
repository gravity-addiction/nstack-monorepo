...
module.exports = env => {
  ...
  // poorly assume root folder is one folder above appPath. usually 'src' or 'projects/projectA/src'
  const projectRoot = resolve(__dirname, appPath, '../');
  ...
  const copyIgnore = { ignore: [`${appResourcesFullPath}/**`] };
  const transformPath = (curPath, abPath) => {
    return curPath.replace('../../../').replace('src/', '');
  }

  const copyTargets = [
    { from: '../../../src/assets/**', noErrorOnMissing: true, globOptions: { dot: false, ...copyIgnore }, transformPath },
    { from: '../../../src/fonts/**', noErrorOnMissing: true, globOptions: { dot: false, ...copyIgnore }, transformPath },
    // config.json is required
    { from: 'assets/**', noErrorOnMissing: false, globOptions: { dot: false, ...copyIgnore }, transformPath },
    { from: 'app.css', noErrorOnMissing: true, to: '.', flatten: true },
    { from: 'app.scss', noErrorOnMissing: false, to: '.', flatten: true },
    ...copyReplacements
  ];
      ...
      rules: [
        ...
        { test: /\.css$/, exclude: /[\/|\\]app\.css$/, use: 'raw-loader' },
        {
          ...
          use: ['raw-loader', 'resolve-url-loader', {
            ...
            options: {
              sassOptions: {
                includePaths: [appFullPath, "./", "../node_modules"]
              }
            }
          }]
        },
