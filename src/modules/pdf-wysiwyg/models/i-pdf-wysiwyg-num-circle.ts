...
export class IPdfWysiwygNumCircle {
  ...
  public draw(ctx: any, selected = false, _showInfo?: string) {
    const lineW = this.stroke || 1;
          ...
          const y = this.y;
          const r = this.r;
          const nW = (this.r * 2) + lineW + this.pR + this.pL;
          const nH = (this.r * 2) + lineW + this.pT + this.pB;
  ...
  public hasCollision(x: number, y: number): boolean {
    const lineW = this.stroke || 1;
          ...
          const left = this.x - this.r - (lineW / 2) - this.pL;
          const right = this.x + this.r + lineW + this.pR;
          const top = this.y - this.r - (lineW / 2) - this.pT;
          const bottom = this.y + this.r + lineW + this.pB;
